...
parser.add_argument("-m_p", "--model_path", type=str, default="/kaggle/working/models", help = "Path to the folder containing the models")
parser.add_argument("-o_p", "--output_path", type=str, default="/kaggle/working/models", help = "Path to the folder where the results will be saved")
parser.add_argument("-e_s", "--experiment_str", type=str, default="Segmentation", help = "String to identify the experiment")
...
parser.add_argument("-n_sigma", "--n_sigma", type=int, default=4, help = "Number of sigma channels, must be at least 1")  # NOTE: was 2
...
parser.add_argument("-layers", "--layers", type=str, default="[32, 64, 128, 256]", help = "UNet layers")  # NOTE: TODO
...
parser.add_argument('-binary_loss_fn', '--binary_loss_fn', default="lovasz_hinge", type=str, help = "Loss function to use for instance segmentation: lovasz_hinge or dice_loss are supported. lovasz_hinge is a lot slower to start converging")  # NOTE: TODO
...
parser.add_argument('-augmentation_type', '--augmentation_type', default="brightfield_only", type=str, help = "'minimal' or 'heavy' or 'brightfield_only'")
...
parser.add_argument('-rng_seed', '--rng_seed', default=42, type=int, help = "Optional seed for the random number generator")  # NOTE: was None
    ...
    for epoch in range(num_epochs):
        ...
        if args.kaggle_username is not None and args.kaggle_key is not None:
            if kagglehub is None:
                raise ModuleNotFoundError("kagglehub is required for Kaggle model uploads. Install it with 'pip install kagglehub'.")
...
def instanseg_training(segmentation_dataset: Dict = None, **kwargs):
    ...
    # setting up random numbers and stuff
    ...
    # configuring other stuff (optimizer, lr scheduler, out_folder and log files)
